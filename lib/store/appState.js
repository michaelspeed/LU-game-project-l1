...
import {generateCanyons} from "../canyon.js";
...
// Initializing the app state, random generators, for dynamics
export function initApplication(appState, jumpDelta) {
  ...
  const canyons = generateCanyons(3)
  return {
    ...appState,
    clouds,
    mountains,
...
// Proxy handler for the app state
export function dispatcher(target, p, newValue, receiver) {
  target[p] = newValue
  return true
}

// Commit function to update the app state
